...
This is my first repository, my first publicly available code, and it is associated with my first R-related publication. 

This repository is for some of the work done in the following publication:
...
The Figure_S1.R file will import the raw qPCR and melt curve files from a relative location, which requires the files themselves and the modification of the file location. This file expects the raw data to be located in an "Input" folder located at the same level as the working directory for the current R session. Similarly, the AA10 file is expected at the same level and should be imported before the rest of the code. Then it will perform all the analyses associated with the data, including preliminary statistics, multiple-comparison t-tests, final graphs and tables and export files.

The Figure_S5B.R file has the same data source and thus the import step can be skipped if the Input_list and AA10 files already exist. The related experiment was only performed once and only a single t test was performed, buried between two large ggplot code blocks.
